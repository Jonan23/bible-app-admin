<script lang="ts">
  ...
  import { ArrowLeft, Save, Send, Loader2 } from 'lucide-svelte';

  let postId = $derived($page.params.id!);
  let isEdit = $derived(!!postId);
  ...
  function autoSlug() {
    if (!isEdit && title) slug = slugify(title);
  }
  ...
  async function handleSave(publish: boolean) {
    ...
    saving = true;
    try {
      const data = { title, slug: slug || slugify(title), content, excerpt: excerpt || undefined, featuredImage: featuredImage || undefined, categoryId: categoryId || undefined, tags: selectedTags, status: publish ? 'PUBLISHED' as const : 'DRAFT' as const, featured };
      if (isEdit) {
        await api.blog.update(postId, data);
      } else {
...
<div class="max-w-3xl mx-auto space-y-6">
  <div class="flex items-center justify-between">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-xl font-bold text-foreground">{isEdit ? 'Edit Post' : 'New Blog Post'}</h1>
        <p class="text-sm text-muted-foreground">{isEdit ? 'Update your blog content' : 'Create a new blog post'}</p>
      </div>
    </div>
  ...
  {#if loading}
    <div class="space-y-4">
      {#each Array(6) as _}<div class="h-10 rounded-xl bg-muted animate-pulse" />{/each}
    </div>
  {:else}
    <div class="space-y-4">
      <div>
      ...
      <div>
        ...
        <textarea
          ...
          rows="2"
          class="w-full px-4 py-3 rounded-xl border border-input bg-background text-sm text-foreground
      ...
      <div>
        ...
        <div class="flex gap-2">
          <input
            ...
            class="flex-1 h-11 px-4 rounded-xl border border-input bg-background text-sm text-foreground
                   placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
          />
          <UploadButton bind:value={featuredImage} label="Upload" accept="image/*" />
        </div>
        {#if featuredImage}
          <img src={featuredImage} alt="preview" class="mt-2 h-32 w-auto rounded-lg object-cover border border-border" />
        {/if}
      </div>
